...
function move(){//basic colllision logic
  ...
  if (x0 + sizeX0 >= width || x0 <= 0){//change direction if the object is within size of the edge of x
    ...
    x0 += dx0*speed*round(deltaTime/((1/refresh)*815))
    tint(random(128,255),random(128,192),random(64,192))//randomly tint the image
  }
  x0 += dx0*speed*round(deltaTime/((1/refresh)*815));//increase x by dx every time the screen redraws
  
  if (y0 + sizeY0 >= height || y0 <= 0){//change direction if the object is within size of the edge of y
    dy0=-dy0;//invert y direction
    y0 += dy0*speed*round(deltaTime/((1/refresh)*815))
    tint(random(128,255),random(128,192),random(128,192));//randomly tint the image
  }
  y0 += dy0*speed*round(deltaTime/((1/refresh)*815));//move in y axis
